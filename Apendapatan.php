...
$sql = "SELECT MONTH(tanggal_paket) AS bulan, SUM(jumlah_orang) AS pelanggan, SUM(total_harga) AS pendapatan
         FROM paket 
         INNER JOIN booking ON paket.id_paket = booking.id_paket
         WHERE YEAR(tanggal_paket) = '$year' AND status_bayar = 'sudah bayar'
         GROUP BY MONTH(tanggal_paket)";
...
// SQL untuk Data pendapatan dalam tabel
$sql2 = "SELECT MONTH(tanggal_paket) AS bulan, SUM(jumlah_orang) AS pelanggan, SUM(total_harga) AS pendapatan
         FROM paket 
         INNER JOIN booking ON paket.id_paket = booking.id_paket
         WHERE YEAR(tanggal_paket) = '$year' AND status_bayar = 'sudah bayar'
         GROUP BY MONTH(tanggal_paket)";
